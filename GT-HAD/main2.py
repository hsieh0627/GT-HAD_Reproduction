...
save_dir = os.path.join(script_dir, '../../results_main2/')
...
def main(file):
    ...
    search_index = torch.arange(0, data_num).type(torch.cuda.LongTensor if torch.cuda.is_available() else torch.LongTensor)
    
    avgpool_small = nn.AvgPool3d(kernel_size=(3, 3, 3), stride=(1, 1, 1), padding=(1, 1, 1)).to(device)
    avgpool_mid = nn.AvgPool3d(kernel_size=(5, 3, 3), stride=(1, 1, 1), padding=(2, 1, 1)).to(device)
    avgpool_large = nn.AvgPool3d(kernel_size=(7, 5, 5), stride=(1, 1, 1), padding=(3, 2, 2)).to(device)
    # start train
    start = time.time()
    for iter in range(1, end_iter + 1):
        ...
        for idx, batch_data in enumerate(data_loader):
            optimizer.zero_grad()
            # input -> net -> output
            net_gt, net_input, block_idx = batch_data['block_gt'].to(device), batch_data['block_input'].to(device), batch_data['index'].to(device)
        ...
        if iter == end_iter:
            ...
            for idx, data in enumerate(infer_loader):
                ...
                res_small = avgpool_small(infer_res)
                res_mid = avgpool_mid(infer_res)
                res_large = avgpool_large(infer_res)
                
                infer_res = (res_small + res_mid + res_large) / 3.0
            ...
            map_path = os.path.join(save_subdir, "GT-HAD2_map.mat")
            sio.savemat(map_path, {'show': residual_np})
            roc_path = os.path.join(save_subdir, "GT-HAD2_roc.mat")
            sio.savemat(roc_path, {'PD': tpr, 'PF': fpr})

            return

if __name__ == "__main__":
    for file in ['gulfport']: 
